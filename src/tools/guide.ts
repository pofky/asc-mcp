...
const FLOWS: Record<Exclude<GuideTopic, "overview" | "limitations">, Flow> = {
  setup: {
    ...
    steps: [
      ...
      "Run `asc-mcp init` in a terminal. It auto-detects the key, asks for your Issuer ID (the UUID at the top of the Integrations page) and an optional Pro license key, and prints a paste-ready MCP config block.",
      "Paste the block into your client config (~/.claude/settings.json, Claude Desktop config, Cursor, etc.) and restart the client.",
